...
\end{theorem}
Furthermore, we will show that the convergence is quadratic.
\begin{proof}
First, since $f$ is twice continuously differentiable around the root $\alpha$, we can use a Taylor series expansion to second order about a point close to $\alpha$, $x_{n}$, to represent $f(\alpha)$. The expansion of $f(\alpha)$ about $x_{n}$ is 
\begin{align}
...
\begin{align}
  \alpha - x_{n+1} = -(\alpha - x_{n})^2 \frac{f''(\xi_{n})}{2f'(x_{n})}
\end{align}
In progr.
%pick a sufficiently small interval $I = [\alpha - \epsilon, \alpha + \epsilon]$ on which $f'(x) \neq 0$ (which exists since $f'(x)$ is continuous) and then . \parencite{rootApproxMeth}
\end{proof}
...
Lastly,%problem s pomalsou konvergenciou
