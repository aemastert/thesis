...
\thesissetup{
    ...
    abstract      = {Solving polynomials is one of the oldest mathematical problems. Over the centuries, there have been many discoveries which led to fast and efficient root finding methods. With the progress of technology, we are now able to solve high degree polynomials in a very short time, using these well known algorithms. In this thesis we present methods for finding root mainly using numerical approximation. We also implement these methods in programming language Java. Lastly, we compare the implemented methods by solving a set of polynomials and comparing computation time. },
...
The requirement that the polynomial function $f(x)$ has no multiple roots does not restrict the generality of the theorem, because in the case that polynomial contains multiple roots, we can first apply square-free factorization and then isolate the roots of the square-free factor \parencite{vincentA}.
...
Thus Algorithm \ref{alg:vas} returns intervals such each contains exactly one positive root. To isolate the negative intervals, we simply substitute $x\leftarrow -x$, execute the algorithm again and then unite the sets of intervals. Then we can use one of the approximation algorithms (e.g. Newton's algorithm (\ref{alg:newton})) and approximate the root in every interval.
...
%	\begin{class}[text width=6cm]{Mobius}{0, 0}
%		\attribute{- a : BigDecimal}
%		\attribute{- b : BigDecimal}
%		\attribute{- c : BigDecimal}
%		\attribute{- d : BigDecimal}
%		
%		\operation{+ getA ( ) : BigDecimal}
%		\operation{+ getB ( ) : BigDecimal}
%		\operation{+ getC ( ) : BigDecimal}
%		\operation{+ getD ( ) : BigDecimal}
%		\operation{+ getValue ( x : BigDecimal , scale : int) : BigDecimal}
%		\operation{+ getValueInfinity ( b : BigDecimal , scale : int) : BigDecimal}
%	\end{class}
%\end{tikzpicture}
