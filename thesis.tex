...
\end{theorem}
\begin{proof}
\textit{(Skecth)} First, since $f$ is twice continuously differentiable around the root $\alpha$, we can use a Taylor series expansion \parencite[p.~59]{rootApproxMeth} of second order about a point close to $\alpha$, $x_{n}$, to represent $f(\alpha)$. The expansion of $f(\alpha)$ about $x_{n}$ is 
\begin{align}
...
\end{align}
for $x_{n+1}$. Following \cite{halley} and simplifying the equation to express $x_{n+1}$ on the left side we obtain
\begin{align}
...
\chapter{Finding multiple roots}
While the previous chapter focused on getting better approximation of a single root, this chapter focuses on finding multiple roots.
\section{Finding all roots}
\subsection{Ruffini-Horner's method}
...
\section{Isolation of roots in a selected interval}
In this section, I present 3 algorithms which goal is to isolate the roots in a selected interval into smaller separate intervals, each containing exactly one root. Then, using the combination of algorithms presented in the previous chapter, approximate the roots to the allowed tolerance, thus achieving the desired result.
...
\end{itemize}
\end{definition}
The Sturm chain of $f$ can be created by applying Euclid's algorithm to $f(x)$ and $f'(x)$ \parencite{sturmWik}\parencite{vigklasphd}:
\begin{align*}
 f_{0}(x)&=f(x), \\
 f_{1}(x)&=f'(x), \\
...
To provide the Sturm's theorem, let $\sigma(f,\xi)$ represent the number of sign changes (change to or from $0$ excluded) in the sequence $f_{0}(\xi), f_{1}(\xi),\ldots,f_{m}(\xi)$, where $f_{0}, f_{1},\ldots,f_{m}$ is Sturm chain of $f$.
...
\end{remark}
Using this theorem and combining it with Newton's method and bisection, I created the following algorithm.
\begin{algorithm}
